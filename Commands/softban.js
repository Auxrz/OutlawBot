        ...
        function softban() {
            ...
            if(!tagged) {
                ...
                if(reason) {
                    ...
                    user.update({$push: {kickreasons: `${reason} | Softbanned by ${message.author.tag}`}},
                    function(err) {
                        if(err)
                        console.log(err) 
                    })
            ...
            async function usersoftban() {
                dauser.ban()
                aguild.unban(user)
                dauser.send(`You have been softbanned for ${reason}`)
            embed.addField('Softban', `${dauser.displayName} has been vanquished.`)
            ...
            message.channels.cache.get(logs).send(logembed)
                ...
                logembed.addField('Actions', `${author.tag} has softbanned ${tagged.displayName}`)
                if(reason) {
                    logembed.addField('Reason', `${reason}`)
                    user.update({$push: {kickreasons: `${reason} | Softbanned by ${message.author.tag}`}},
                    function(err) {
                ...
                if(!tagged.kickable) return message.channel.send('I cannot softban this user.')
                if(user.Kicks === null) {
                    user.update({$set: {Kicks: '1'}},
                        function(err) {
                        if(err)
                ...
                embed.addField('Softban', `${tagged.displayName} has been vanquished.`)
                message.channel.send(embed)
                if(daguild.logs === null) return
